...
class VisitorController extends Controller
{
    ...
    public function getAdvertise($pubId, $slug, $currentUrl)
    {
        ...
        $adType = AdType::whereSlug($slug)->first();
        ...
        if (isset($adType)) {
            ...
            //     }
            // })->inRandomOrder()->first();
            $ads = CreateAd::where('ad_type_id', $adType->id)->whereStatus(1)->where(function ($q) use ($domain) {
                foreach ($domain->keywords as $keyword) {
                    $q->orWhere('keywords', 'LIKE', "%$keyword%");
                }
            })->inRandomOrder()->first();

            $existIpLog = $existingIp->iplogs()->where('ad_id',$ads->id)->where('time','>=',Carbon::now()->subMinutes($setting->intervals))->count();
            if (isset($ads)) {
    ...
    public function adClicked($publisherId, $trackId)
    {
        ...
        $existIpLog = $existingIp->iplogs()->where('ad_id',$ad->id)->where('time','>=',Carbon::now()->subMinutes($setting->intervals))->count();
        if (isset($ad)) {
